# Food Preferences

Personal food preferences to guide recipe development and meal planning.

---

## Allergies & Restrictions

**Oral Allergy Syndrome (OAS):**
- Raw banana
- Fuzzy fruit (peaches, nectarines, apricots, etc.)
- Almonds

> Note: Cooking typically denatures the proteins that trigger OAS, so cooked versions may be tolerable. Test cautiously.

**Texture/Taste Aversions:**
- Raw tomato (cooked tomato sauces are OK)
- Bitter flavors

---

## Protein Preferences

Ranked by preference and enjoyment cooking:

| Protein | Preference | Notes |
|---------|------------|-------|
| Beef | ★★★★★ | Top choice |
| Pork | ★★★★★ | Top choice |
| Chicken | ★★★★☆ | Solid go-to |
| Fish | ★★★☆☆ | OK, less enjoyable to cook |
| Turkey | ★★★☆☆ | OK, less enjoyable to cook |

---

## Nutrition Goals

**Daily Target:** 1800–2000 calories (cutting weight)

- **High protein** — meat-forward meals preferred
- **High fiber** — for satiety and health
- **High satiety** — filling meals that last
- **More leafy greens** — active goal to incorporate

**Meal Planning Notes:**
- Fine with eating the same meals repeatedly
- Prioritize calorie density awareness for portion planning

---

## Go-To Staple Meals

Reliable fallbacks that require zero thought:

- **PB&J** — always works
- **Costco beef chili** — easy protein-forward option

---

## Vegetables

**Favorites:**
- Napa cabbage
- Potatoes
- Cooked carrots
- Sweet potatoes
- Brussels sprouts

---

## Cuisine Preferences

- American
- Mexican
- Thai
- Japanese
- Korean
- Italian

---

## Lifestyle & Constraints

**Time:** Limited — constantly working, need low-effort cooking

**Budget:** No constraints

**Preferred Cooking Methods:**
- Slow cooker — set it and forget it
- Baked/roasted — hands-off oven time

**Cooking Style:** Meat and potatoes — hearty, satisfying, no-fuss meals

---

## Flavor Profile

- **Spice tolerance:** High — can handle heat
- **Bitter:** Avoid — not enjoyable
- **Umami:** Yes — pantry staples support this (soy sauce, mirin, bouillon)

---

## Pantry Staples

Always stocked:
- White rice
- Brown rice
- Chicken bouillon
- Soy sauce
- Rice wine vinegar
- Sesame oil
- Mirin
- Variety of hot sauces
- Salt
- Pepper

---

## Recipe Development Guidelines

When creating new protocols for this kitchen:

1. **Avoid** raw banana, fuzzy fruits, and almonds in all recipes
2. **Prefer** beef and pork for protein-forward dishes
3. **Prioritize** slow cooker and baked/roasted methods — minimal active time
4. **Design for busy schedules** — low-effort, hands-off cooking
5. **Build hearty meals** — meat and potatoes style, high satiety
6. **Include** fiber sources and leafy greens where possible (goal area)
7. **Lean into** umami-forward, Asian-inspired, or comfort food profiles
8. **Embrace** heat and spice; avoid bitter preparations
9. **Use** pantry staples (soy, mirin, sesame, rice vinegar) for convenience
10. **Sub** raw tomato with cooked/sauce forms when tomato flavor is needed
11. **Include calorie estimates** — target 1800–2000 cal/day total
12. **Design for meal prep** — repeatable meals are fine, batch cooking encouraged
